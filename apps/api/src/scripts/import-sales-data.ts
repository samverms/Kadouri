...
async function importSalesData() {
  ...
  let defaultProduct = await db.select().from(products).where(eq(products.code, 'MISC')).limit(1)
  ...
  for (const [invoiceNum, lineItems] of invoiceMap) {
    try {
      ...
      const [order] = await db.insert(orders).values({
        ...
        sellerId: sellerId,
        buyerId: buyer.id,
        status: 'paid', // Historical data is already paid
        qboDocType: 'invoice',
        qboDocId: invoiceNum, // Use invoice number as ID for now
        qboDocNumber: invoiceNum,
        subtotal: subtotal.toString(),
        commissionTotal: commissionTotal.toString(),
        totalAmount: subtotal.toString(),
        createdBy: 'system', // Historical import
        createdAt: orderDate,
        updatedAt: orderDate,
      }).returning()
