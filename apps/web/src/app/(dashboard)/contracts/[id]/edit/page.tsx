    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({
          ...
          validFrom: new Date(contract.validFrom).toISOString().split('T')[0],
          validUntil: new Date(contract.validUntil).toISOString().split('T')[0],
          brokerName: contract.brokerName || '',
          brokerAddress: contract.brokerAddress || '',
          brokerPhone: contract.brokerPhone || '',
          brokerEmail: contract.brokerEmail || '',
          terms: contract.terms || '',
          notes: contract.notes || '',
        })

        // Set seller
        setSelectedSeller({ id: contract.sellerId, name: contract.seller.companyName, code: contract.seller.code })
        setSellerNameSearch(contract.seller.companyName)
        setSellerCodeSearch(contract.seller.code || '')

        // Set buyer
        setSelectedBuyer({ id: contract.buyerId, name: contract.buyer.companyName, code: contract.buyer.code })
        setBuyerNameSearch(contract.buyer.companyName)
        setBuyerCodeSearch(contract.buyer.code || '')

        // Set product
        setSelectedProduct({ id: contract.productId, name: contract.product.name })
        setProductSearch(contract.product.name)
      }
    } catch (error) {
  ...
  return (
    <div className="p-8">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-gray-900 dark:text-blue-400">Edit Contract</h1>
      </div>
