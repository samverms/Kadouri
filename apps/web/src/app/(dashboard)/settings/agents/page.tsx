...
import { Plus, Search, User, ArrowUpDown, ArrowUp, ArrowDown, Edit2, Trash2, Eye, EyeOff, Filter, FilterX, Mail, Phone, Building2, CheckCircle2, XCircle } from 'lucide-react'
...
export default function AgentsPage() {
  ...
  const filteredAgents = useMemo(() => {
    ...
    if (columnFilters.name) {
      filtered = filtered.filter(agent =>
        agent.name.toLowerCase().includes(columnFilters.name.toLowerCase())
      )
    }
    if (columnFilters.companyName) {
      filtered = filtered.filter(agent =>
        agent.companyName?.toLowerCase().includes(columnFilters.companyName.toLowerCase())
      )
    }
    if (columnFilters.email) {
      filtered = filtered.filter(agent =>
        agent.email?.toLowerCase().includes(columnFilters.email.toLowerCase())
      )
    }
    if (columnFilters.phone) {
      filtered = filtered.filter(agent =>
        agent.phone?.toLowerCase().includes(columnFilters.phone.toLowerCase())
      )
    }
    ...
    return filtered
  }, [agents, searchQuery, activeFilter, columnFilters, sortField, sortDirection])
  ...
  const renderSortIcon = (field: keyof Agent) => {
  ...
  if (isLoading) {
    return (
      <div className="p-6">
        <Card>
          <CardContent className="p-6 text-center">Loading agents...</CardContent>
        </Card>
      </div>
    )
  }
    ...
    <div className="p-6 space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage sales agents</p>
        </div>
        ...
        <CardHeader className="border-b space-y-4">
          <div className="flex items-center gap-4">
            <div className="relative flex-1">
              ...
              <Input
                type="text"
                placeholder="Search agents by name, company, email, or phone..."
                value={searchQuery}
